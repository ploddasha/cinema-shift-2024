...
class ScheduleItemConverter {
    ...
    fun convert(model: ScheduleModel): ScheduleItem =
        ScheduleItem(
            date = model.date,
            seances = model.seanceModels.map { convertSeance(it) }
        )

    private fun convertSeance(model: SeanceModel): SeanceItem =
        SeanceItem(
            time = model.time,
            hall = convertHall(model.hallModel),
            payedTickets = model.payedTicketModels.map { convertPayedTicket(it) }
        )

    private fun convertHall(model: HallModel): HallItem =
        HallItem(
            name = model.name,
            places = model.placeModels.map { row -> row.map { convertPlace(it) } }
        )
